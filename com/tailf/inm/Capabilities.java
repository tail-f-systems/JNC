...
public class Capabilities {
     ...
     */
    public static String NS_NOTIFICATION=
        "urn:ietf:params:xml:ns:netconf:notification:1.0";
     ...
     * "http://tail-f.com/ns/netconf/actions/1.0".
     */
    public static final String ACTIONS_CAPABILITY =
        "http://tail-f.com/ns/netconf/actions/1.0";

    /** String constant for the <code>:transactions</code> capability.
     *  "http://tail-f.com/ns/netconf/transactions/1.0".
     */
    public static final String TRANSACTIONS_CAPABILITY=
        "http://tail-f.com/ns/netconf/transactions/1.0";
     ...
     * <p>
     * The <code>:url</code> capability URI must contain a "scheme"
     * argument assigned a
     * comma-separated list of scheme names indicating which schemes the
     * NETCONF peer supports.
     * If the <code>:url</code> capability is set the supported schemas will
     * listed in {@link #urlSchemes}.
     * <p>
     * If the <code>:url</code> capability is supported an url argument
     * may be given to
     * the follwing methods:
     * <ul>
     * <li> {@link #editConfig(int,String)} - the <edit-config> operation
     * can accept an URL as an alternative to the config tree.
     * The URL should identify a local configuration file.
     * <li> {@link #copyConfig(String,String)} - the <copy-config> operation
     * can accept an URL as either the source or target parameter.
     * <li> {@link #deleteConfig(String)} - the <delete-config> operation
     * can accept an URL, then it should identify a local configuration file.
     * <li> {@link #validate(String)} - the <validate> operation can
     * accept an URL as the source parameter.
     * </ul>
     */
     ...
     * <p>
     * For example:
     * In "urn:ietf:params:netconf:capability:url:1.0?scheme=http,ftp,file"
     * the urlSchemes are: "http","ftp","file"
     */
    ...
    protected Capabilities(Element e) throws INMException {
        ...
        for(int i=0;i<caps.size();i++) {
            ...
            if (uri.equals(NETCONF_BASE_CAPABILITY))
                ...
                urlCapability = true;
                String schemes=
                    cap.value.toString().substring(URL_CAPABILITY_SCHEME.length());
            ...
            /**
             * JUNIPER has an erroneous naming of the capabilities
             *  let's accept their capabilities also.
             */
            else if (uri.equals("urn:ietf:params:xml:ns:netconf:base:1.0"))
                baseCapability = true;
            else if (uri.equals("urn:ietf:params:xml:ns:netconf:capability:candidate:1.0"))
                candidateCapability = true;
            else if (uri.equals("urn:ietf:params:xml:ns:netconf:capability:confirmed-commit:1.0"))
                confirmedCommitCapability = true;
            else if (uri.equals("urn:ietf:params:xml:ns:netconf:capability:validate:1.0"))
                validateCapability = true;
            else if (uri.equals(
     ...
     * if no revision found. Only check the user data capabilities.
     */
