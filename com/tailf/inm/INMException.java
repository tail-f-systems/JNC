 ...
 * <p>
 * INMException uses an errorCode field to indicate what went
 * wrong. Depending on errorCode an opaqueData field may point to
 * contextual information describing the error. The toString method
 * uses both of these fields to print an appropriate error string
...
public class INMException extends Exception {
     ...
     * </ul>
     * <p>
     * Depending on the value the opaqueData field may be set
     * accordingly. If so this is described below for each possible
     * value.
     */
    public int errorCode = NOT_SET;

    /**
     * Contextual information describing the error. The meaning of
     * this field as is described for each possible errorCode value.
     */
    ...
    /**
     * An <code>rpc-reply</code> with one or more
     * <code>reply-error</code> elements was returned from the
     * device. The opaqueData field contains the error (in XML
     * format) returned from the device.
     */
    public static final int RPC_REPLY_ERROR = -6;

    /**
     * A session error occured. The opaqueData field is a string
     * describing the error.
     */
    public static final int SESSION_ERROR = -7;

    /**
     * This element has already been used. The opaqueData field
     * is the offending Element object.
     */
    public static final int ELEMENT_ALREADY_IN_USE = -8;

    /**
     * This element does not exist. The opaqueData field
     * is a path pointing to the missing element.
     */
    ...
    /**
     * Timeout error. The timeout specified when creating the
     * SSHSession object expired.
     */
     ...
     * encountered. This only applies to ConfM code.
     * A revision error occurs when we try to encode something the
     * receiving node doesn't understand, such as an enumeration/bit
     ...
     * we received from the Netconf server didn't match the message-id
     * in the rpc we sent.
     */
      public static final int MESSAGE_ID_MISMATCH    = -13;

    /**
     * The toString method uses both of errorCode and opaqueData
     * fields to generate an appropriate error string.
     */
    ...
    public String toString() {
        ...
        case RPC_REPLY_ERROR:
            if (opaqueData != null)
                return "rpc-reply error: "+((Element)opaqueData).toXMLString();
            else
