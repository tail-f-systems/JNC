 ...
 * a set of {@link Element} nodes that fulfill the
 * location steps, with node tests, and predicates.
 * The path expression operates on an element tree.
 * <p>
 * Example:
 * <pre>
...
public class Path {
     ...
     * <p>
     * Makes a selection by traversing the locationSteps,
     * return an ArrayList (NodeSet) of Element nodes.
     *
     ...
     * <p>
     * Makes a selection by traversing ONE locationStep.
     * Returns an updated NodeSet of Element nodes.
     */
    NodeSet evalStep(NodeSet nodeSet,int step)
        throws INMException {
     ...
     * <ul>
     * <li> an axis, which specifies the tree relationship between
    ...
    class LocationStep {
        ...
        /** perform nodeTest on nodeSet.
         * (only NameTest) since all
         *  nodes are simplified to be Elements
         */
        private NodeSet nodeTest(NodeSet nodeSet) throws INMException {
            NodeSet result = new NodeSet();
            /** A simple "NameTest"
             *  Filter away those with wrong name
             */
            for(int i=0;i<nodeSet.size();i++){
                ...
                if (node.name.equals(name)) {
                    ...
                    if (prefix!=null) {
                        ...
                        if (namespace!=null && node.namespace.equals(namespace))
                            result.add(node);
                    } else {
            ...
            /** apply predicates on nodeSet in order, if any.
             * rebuild new nodeSet, for each predicate level,
             * since contextSet is needed as an argument we
         ...
         * of elements.
         */
        Element createElem(PrefixMap prefixMap, Element parent) throws INMException {
            trace("createElem() from "+this);
            switch(axis) {
            case AXIS_ROOT: return null;
            case AXIS_CHILD:
                String ns=null;
                if (prefix==null)
                    if (parent!=null) ns = parent.namespace;
                    else ns = prefixMap.prefixToNs("");
                else ns = prefixMap.prefixToNs(prefix);
                if (ns==null)
    ...
    class Expr  {
        ...
        public Boolean eval(Element node, NodeSet contextSet) throws INMException {
            return f_boolean( eval2(node,contextSet) );
        }

        private Object eval2(Element node, NodeSet contextSet) throws INMException {
            ...
            case OR:
                ...
                if ( f_boolean(lval).booleanValue()) return new Boolean(true);
                else return f_boolean(rval);
            case AND:
                // rvalue not evaluated if lval is 'false'
                if ( ! f_boolean(lval).booleanValue() ) return new Boolean(false);
                else return f_boolean(rval);
            case EQ:   // '='
            case NEQ:  // '!='
                if (isBoolean(lval)) rval = f_boolean(rval);
                else if (isBoolean(rval)) lval = f_boolean(lval);
                else if (isNumber(lval)) rval = f_number(rval);
                else if (isNumber(rval)) lval = f_number(lval);
                else {
                    lval = f_string(lval);
                    rval = f_string(rval);
                }
                if (op==EQ)
                    if (compare(lval,rval)==0) return new Boolean(true);
                    else return new Boolean(false);
                else // op==NEQ
                    if (compare(lval,rval)!=0) return new Boolean(true);
                    else return new Boolean(false);
            case GT:   // '>'
                if (compare( f_number(lval), f_number(rval))>0) return new Boolean(true);
        ...
        private int compare(Object x,Object y) throws INMException {
            if ((x instanceof Boolean) && (y instanceof Boolean))
                if (((Boolean)x).booleanValue() == ((Boolean)y).booleanValue())
                    return 0;
                else return -1;
            ...
            if ((x instanceof String) && (y instanceof String))
                if (((String)x).equals((String)y)) return 0;
                else return -1;
            if (x==null) return -1;
            if (y==null) return +1;
            throw new INMException(INMException.PATH_ERROR,
                                    "badarg to compare (not of same type): "
                                    +x+", "+y);
        }
        ...
        private Boolean f_boolean(Object x) throws INMException {
            if (x instanceof Float)
                if (((Float)x).floatValue()>0 || ((Float)x).floatValue()<0)
                    return new Boolean(true);
                else return new Boolean(false);
            if (x instanceof Integer)
                if (((Integer)x).intValue()>0 || ((Integer)x).intValue()<0)
                    return new Boolean(true);
                else return new Boolean(false);
            if (x instanceof String)
                if ( ((String)x).length() >0) return new Boolean(true);
                else return new Boolean(false);
        ...
        private Number f_number(Object x) throws INMException {
            ...
            if (x instanceof Boolean)
                if (((Boolean) x).booleanValue()) return new Integer(1);
            ...
            if (x instanceof NodeSet) return f_number( f_string(x) );
            if (x==null) return null;
            throw new INMException(INMException.PATH_ERROR,
        ...
        private NodeSet f_nodeSet(Object x) throws INMException {
            if (x instanceof NodeSet) return (NodeSet) x;
            if (x==null) return null;
            throw new INMException(INMException.PATH_ERROR,
        ...
        private String f_string(Object x) throws INMException {
            if (x instanceof Integer) return ((Integer)x).toString();
            if (x instanceof Float) return ((Float)x).toString();
            if (x instanceof String) return (String)x;
            if (x instanceof Boolean)
                if (((Boolean)x).booleanValue()) return new String("true");
         ...
         */
        Object evalCreate(Element node)
            throws INMException {
        ...
        public String toString() {
            String s= "";
            switch (op) {
            case AND: return "AND("+lvalue+","+rvalue+")";
            case OR:  return "OR("+lvalue+","+rvalue+")";
            case EQ:  return "EQ("+lvalue+","+rvalue+")";
            case NEQ: return "NEQ("+lvalue+","+rvalue+")";
            case GT:  return "GT("+lvalue+","+rvalue+")";
            case GTE: return "GTE("+lvalue+","+rvalue+")";
            case LT:  return "LT("+lvalue+","+rvalue+")";
            case LTE:          return "LTE("+lvalue+","+rvalue+")";
            case PLUS:         return "PLUS("+lvalue+","+rvalue+")";
            case MINUS:        return "MINUS("+lvalue+","+rvalue+")";
            case FUN_BOOLEAN:  return "FUN_BOOLEAN("+lvalue+")";
            case FUN_NUMBER:   return "FUN_NUMBER("+lvalue+")";
            case FUN_STRING:   return "FUN_STRING("+lvalue+")";
            case FUN_POSITION: return "FUN_POSITION()";
            case FUN_LAST:     return "FUN_LAST()";
            case FUN_COUNT:    return "FUN_COUNT("+lvalue+")";
            case FUN_CONCAT:   return "FUN_COUNT("+lvalue+","+rvalue+")";
            case FUN_NOT:      return "FUN_NOT("+lvalue+")";
            case FUN_TRUE:     return "FUN_TRUE()";
            case FUN_FALSE:    return "FUN_FALSE()";
            case FUN_NEG:      return "FUN_NEG("+lvalue+")";
            case ATTR_VALUE:   return "ATTR_VALUE("+lvalue+")";
            case CHILD_VALUE:  return "CHILD_VALUE("+lvalue+")";
            default:
     ...
     *
     */
    ArrayList parse(TokenList tokens)
        throws INMException {
        ...
        try {
            ...
            while (sz>0) {
                trace("parse(): "+tokens);
                /* peek at tokens */
                if (sz>=1) tok1= tokens.getToken(0);
                else tok1= new Token();
                if (sz>=2) tok2= tokens.getToken(1);
                else tok2= new Token();
                if (sz>=3) tok3= tokens.getToken(2);
                else tok3= new Token();
                if (sz>=4) tok4= tokens.getToken(3);
                else tok4= new Token();
                if (sz>=5) tok5= tokens.getToken(4);
                else tok5= new Token();
                ...
                /* AXIS::PREFIX:TAG */
                else if (tok1.type==ATOM && tok2.type==COLONCOLON && tok3.type==ATOM &&
                         tok4.type==COLON && tok5.type==ATOM) {
                    step = new LocationStep( parseAxis(tok1.value), tok3.value, tok5.value );
                    steps.add(step);
                ...
                /* PREFIX:TAG */
                else if (tok1.type==ATOM && tok2.type==COLON && tok3.type==ATOM) {
    ...
    int parseAxis(String str) throws INMException {
        if (str.equals("child")) return AXIS_CHILD;
        if (str.equals("self")) return AXIS_SELF;
        throw new INMException(INMException.PATH_ERROR,
     ...
     */
    void parsePredicates(TokenList tokens, LocationStep step) throws INMException {
        ...
        if (sz>=1) {
            ...
            if (tok1.type==LPRED) {
                ...
                int i=1;
                try { while( (tokens.getToken(i)).type!=RPRED) i++;
                } catch (Exception e) {
                    throw new INMException(INMException.PATH_ERROR,
    ...
    Expr parsePredicate(TokenList tokens,int from,int to)
        ...
        while (i<to) {
            ...
            if ((i+1)<to) tok2= tokens.getToken(i+1);
            else tok2= null;
            if ((i+2)<to) tok3= tokens.getToken(i+2);
            else tok3= null;
            ...
            if (tok1.type==ATOM && tok2.type==COMPARE && tok3!=null) {
                ...
                return new Expr(tok2.op, new Expr(CHILD_VALUE,tok1.value), rexpr);
            }
            /* ATTR = */
            else if (tok1.type==ATTR && tok2.type==COMPARE && tok3!=null) {
                Object rexpr= parsePredicate_rvalue(tokens,i+2,to);
                return new Expr(tok2.op, new Expr(ATTR_VALUE,tok1.value), rexpr);
            }
            /* It's a parse error */
            else
    ...
    void parseError(TokenList tokens,int from, int to) throws INMException {
            ...
            for (int i=from;i<to;i++) {
                if (i<sz) errStr=errStr+ tokens.getToken(i).value;
                else break;
            }
            errStr= errStr+"...\"";
    ...
    class Token {
        ...
        public String toString() {
            ...
            case OP:         return "OP("+value+")";
            case COMPARE:    return "COMPARE("+value+")";
            case ATOM:       return "ATOM("+value+")";
            case ATTR:       return "ATTR("+value+")";
            case NUMBER:     return "NUMBER("+value+")";
            case STRING:     return "STRING("+value+")";
            case LBRACER:    return "LBRACER";
            case RBRACER:    return "RBRACER";
            case LPRED:      return "LPRED";
            case RPRED:      return "RPRED";
            case COMMA:      return "COMMA";
            default:    return "UNKOWN(type="+type+",value="+value+")";
            }
        }
    }
    ...
    TokenList tokenize(String s) throws INMException {
        ...
        while(i<s.length()) {
            ...
            if ((i+1)<buf.length) next= buf[i+1];
            else next = 0;
            curr = buf[i];
            if (curr==' ' || curr=='\t' || curr== '\n') {
                // whitespace - ignore
                ...
                if (j==buf.length)
                    throw new INMException(INMException.PATH_ERROR,
                                           "unterminated value: "+new String( buf,i,buf.length-i));
                ...
                case '=':
                    if (next == '>') {
                        ...
                        i++;
                    } else tokens.add( new Token(COMPARE,EQ, "="));
                    break;
                case '>':
                    if (next=='=') {
                        tokens.add(new Token(COMPARE,GTE,">="));
                        i++;
                    } else tokens.add( new Token(COMPARE,GT, ">"));
                    break;
                case '<':
                    if (next=='=') {
                        tokens.add(new Token(COMPARE,LTE, "<="));
                        i++;
                    } else tokens.add( new Token(COMPARE,LT, "="));
                    break;
                default:
                    throw new INMException(INMException.PATH_ERROR,
