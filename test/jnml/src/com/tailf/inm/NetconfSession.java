 ...
 * transport mechanism. After a successful connect all
 ...
 * <code>commit</code>.
 * <p>
 * The NETCONF session class is data model agnostic. Manipulation of
 * data is done via the {@link Element} class.
 * <p>
 * Example 1: Read and manipulate data locally
 * <p>
 * A get-config operation with an XPATH filter is sent to the
 * agent (the agent must implement the <code>:xpath</code>
 * capability). When the configuration tree is received it is edited
 * locally and written 
 * back with an <code>edit-config</code> operation.
 * The read and write is done towards the RUNNING datastore.
 * <pre>
 ...
 * <p>
 * A subtree filter is built using the <code>Element.create</code> method.
 * The matching configuration is received and one of the elements is deleted.
 * It is "marked" for deletion and an <code>edit-config</code>
 * operation is used to actually delete the element on the agent.
 * <pre>
...
public class NetconfSession {
    ...
    /**
     * Session identifier. 
     * Received from the initial <code>hello</code> message.
     */ 
    public int sessionId;

    /**
     * The capability elements for the session.
     * Retrieved from <code>hello</code> message upon connect.
     */ 
     ...
     * The capabilities are received from the <code>hello</code>
     ...
     * This will initialize the transport and
     * send out an initial hello message to the server.
     * @see SSHSession
     *
     * @param transport Transport object 
     */

    public NetconfSession( Transport transport ) 
     ...
     * objects we must instantiate the parser as an com.tailf.confm.XMLParser() 
     * If we fail to do that, we get the XMLParser class from the inm 
     * package which always only returns Element objects as opposed to
     * the XMLParser from the confm package which can return objects
     * matching the confm generated classes.
     * If we use the confm Device class to create our netconf sessions, this
     * is all done automatically, whereas if we create our netconf sessions
     * ourselves, _and_ also want to retrieve real confm objects we must:
     * <pre>
    ...
    /**
     * Creates a new session object.
     * The session need to be given a transport object
     * with {@link #setTransport(Transport)} and
     * an initial hello needs to be sent
     * to advertise capabilities.
     *
     */
    public NetconfSession() throws INMException {
	parser = new XMLParser();
    }
    ...
    /**
     * Capabilities are advertised in messages sent by 
     * each peer during session establishment. When the 
     * NETCONF session is opened, each peer (both client
     * and server) must send a <code>hello</code> element containing a
     * list of that peer's capabilities. Each peer must 
     * send at least the base NETCONF capability, 
     * "urn:ietf:params:netconf:base:1.0".
     * <p>
     * This method will send an initial <code>hello</code> to
     * the output stream and await the <code>hello</code> from the
     * server.
     * <p>
     * Used from the constructor {@link #NetconfSession(Transport)}.
     */
    protected void hello() throws INMException, IOException {
	trace("hello: ");	
	encode_hello(out);
	out.flush();
     ...
     * (RPC) paradigm.  A client encodes an RPC in XML 
     * and sends it to a server using a secure, 
     * connection-oriented session.  The server responds with a
     * reply encoded in XML.  
     * <p>
     * This method may be used for sending an XML string over
     * the connected session and receiving a reply.
     * <p>
     * The reply is parsed into an element tree.
     *
     * @param request XML encoded NETCONF request
     */        
    public Element rpc(String request) 
     ...
     * reply encoded in XML.  
     * <p>
     * This method may be used for sending an XML element tree
     * the connected session and receiving a reply.
     * <p>
     * The reply is parsed into an element tree.
     *
     ...
     */        
    public Element rpc(Element request) 
	throws IOException, INMException {
     ...
     * <p>
     * Note that the return value reflects the last request-id
     * generated by the NetconfSession methods, and is not checked
     * against the request-id in the request parameter.
     *
     * @param request XML encoded NETCONF request
     */
    ...
    /**
     * Sends rpc request and return.
     * This method may be used for sending an XML element tree over
     * the connected session.
     * To receive a reply the {@link #readReply()} should be used.
     * <p>
     * Note that the return value reflects the last request-id
     ...
     */
    public int sendRequest(Element request) 
	throws IOException, INMException {
	// print, but no newline at the end
	request.encode(out,false, capabilities);
	out.flush();
     ...
     * Preforms a blocking read.
     * <p>
     * The reply is parsed into an element tree.
     *
     * @see #sendRequest(Element)
     */
    public Element readReply() 
	throws IOException, INMException {
	StringBuffer reply = in.readOne();
	return 	parser.parse( reply.toString() );
    }
     ...
     */
    public NodeSet getConfig(Element subtreeFilter) 
	throws INMException, IOException {	
     ...
     * 
     */
    public NodeSet getConfig(int datastore) 
	throws INMException, IOException {	
     ...
     * 
     */
    public NodeSet callRpc(Element data) 
	throws INMException, IOException {	
     ...
     * Gets the device configuration data specified by
     * an xpath expression.
     * The <code>:xpath</code> capability must be supported by the server.
     *
     * @param xpath XPath expression 
     */
    public NodeSet getConfig(String xpath)
	throws INMException, IOException {
     ...
     * One of {@link #RUNNING}, {@link #CANDIDATE}, {@link #STARTUP}
     * @param subtreeFilter A subtree filter
     */
	...
	int mid = encode_getConfig(out, encode_datastore(datastore), subtreeFilter);
	out.flush();
	return recv_rpc_reply_data(mid);
    }
     ...
     * @param subtreeFilter A subtree filter
     */
    public NodeSet get(Element subtreeFilter) 
	throws INMException, IOException {
     ...
     */
    public NodeSet get(String xpath) 
	throws INMException, IOException {
	trace("get: \""+xpath+"\"");
     ...
     * configuration to the {@link #RUNNING} target datatore.
     * @param configTree Configuration tree
     */
    public void editConfig(Element configTree) 
	throws INMException, IOException {
     ...
     * NodeSet as opposed to an Element to the device
     */

    public void editConfig(NodeSet configTrees) 
	throws INMException, IOException {
	...
	int mid = encode_editConfig(out, encode_datastore(datastore),configTree);
	out.flush();
	...
	int mid = encode_editConfig(out, encode_datastore(datastore),configTrees);
	out.flush();
	recv_rpc_reply_ok(mid);
    }
     ...
     */
    public void editConfig(int datastore, String url) 
	throws INMException, IOException {
	trace("editConfig: target="+datastoreToString(datastore)+
	      " source="+url);
	int mid = encode_editConfig(out, encode_datastore(datastore), encode_url(url));
	out.flush();
	recv_rpc_reply_ok(mid);
    }
     ...
     * a different operation.  If the configuration parameter contains 
     * data for which there is not a
     ...
     * <li>
     * {@link #TEST_ONLY}: Only test. 
     * (Option is not supported in the standard.) 
     * </ul>
     * @param testoption 
     * One of {@link #SET}, {@link #TEST_THEN_SET}, {@link #TEST_ONLY}
     */
     ...
     * the server to support the <code>:rollback-on-error</code> capability.
     * </ul>
     * @param erroroption 
     ...
     * Only possible if <code>:url</code> capability is supported.
     *
     * @param sourceTree A config tree
     ...
     * @param target The target datastore
     */
    public void copyConfig(int source, int target) 	
	throws INMException, IOException {
     ...
     * @param targetUrl The target URL.
     */
    public void copyConfig(int source, String targetUrl) 
	throws INMException, IOException {
	trace("copyConfig: source="+datastoreToString(source)
	      +" target="+targetUrl);
    ...
    /**
     * Same as {@link #copyConfig(int,int)} but
     * uses an url as target and an url as a source.
     * Only possible if <code>:url</code> capability is supported.
     *
	...
	throws INMException, IOException {
	trace("copyConfig: source="+sourceUrl
	      +" target="+targetUrl);
     ...
     * @param target The target datastore
     */
    public void copyConfig(String sourceUrl, int target) 
	throws INMException, IOException {
	trace("copyConfig: source="+sourceUrl
	      +" target="+datastoreToString(target));
     ...
     */
    public void deleteConfig(int datastore) 
	throws INMException, IOException {
     ...
     */
    public void deleteConfig(String targetUrl) 
	throws INMException, IOException {
     ...
     */
    public void lock(int datastore)  
	throws INMException, IOException {
     ...
     */
    public void unlock(int datastore) 
	throws INMException, IOException {
     ...
     * data store.  The portion to lock is specified by an array of selection
     * strings which can be XPath filters (if the server supports XPath) 
     * or a simple paths to instance idenfifiers.
     * <p>
     * If the selection filters are XPath expressions they are evaluated 
     * only once at lock time, thereafter the scope of the lock 
     * is maintained as a set of nodes.  If
     ...
     * 
     */
    public int lockPartial(String[] select)  
	throws INMException, IOException {
	trace("lockPartial");
	...
	try {	    
	    ...
	    throw new INMException(
                INMException.SESSION_ERROR,
                "bad lock-id returned from partial-lock: "+reply.toXMLString());
	}
    }
     ...
     */
    public void unlockPartial(int lockId) 
	throws INMException, IOException {
     ...
     * current configuration, use the <code>commit</code> operation.
     * <p>
     ...
     * 
     */
    public void commit() throws INMException, IOException {
	trace("commit");
     ...
     * 
     * @param timeout Time that server will wait for confirming commit 
     * before reverting config
     *
     */
    public void confirmedCommit(int timeout) throws INMException, IOException {
     ...
     * 
     */
    public void discardChanges() throws INMException, IOException {
     ...
     * <p>
     * When a NETCONF server receives a <code>close-session</code> request, 
     * it will
     * gracefully close the session.  The server will release any locks
     * and resources associated with the session and gracefully close any
     * associated connections.
     */
    public void closeSession() throws INMException, IOException {
	trace("closeSession");
     ...
     * if this value is the current session ID a INMException will be
     * thrown.
     * @param sessionId The id of the session to terminate
     */
    public void killSession(int sessionId) throws INMException, IOException {
     ...
     */
    public void validate(Element configTree) 
	throws INMException, IOException {
     ...
     */
    public void validate(int datastore) throws IOException, INMException {
     ...
     * The url may for example be a file 
     * <code>"file://incoming.conf"</code>
     * then file must be supported by the <code>:url</code> capability.
     * 
     * @param url The source url to validate
     */
    public void validate(String url) throws IOException, INMException {
	trace("validate: "+url);
        if (!capabilities.hasValidate())
	    throw new INMException(
                INMException.SESSION_ERROR,
     ...
     * notifications specified in a subscription.
     * The <code>:notification</code> capability must be supported 
     * by the server.
     * @see #createSubscription(String,String,String,String)
     * 
     */
    public void createSubscription() throws IOException, INMException {
     ...
     * by the server.
     * <p>
     * An optional parameter, <code>stream</code>, that indicates 
     * which stream of events is of interest.  If not present, 
     * events in the default NETCONF stream will be sent.
     * <p>
     * An optional parameter, <code>filter</code>, that indicates 
     * which subset of all possible events is of interest.  
     * If not present, all events not precluded by other parameters 
     * will be sent.  
     * <p>
     *  A parameter, <code>startTime</code>, used to trigger the replay 
     * feature and indicate that the replay should start at the time
     * specified.  If <code>startTime</code> is not present, this is not 
     * a replay subscription.  It is not valid to specify start times that are
     * later than the current time.  If the <code>startTime</code> specified is
     ...
     * with and be later than <code>startTime</code>.  
     * Values of <code>stopTime</code> in the future are valid.
     * The time is specified in dateTime format.
     * <p>
     *
     ...
     * @param stopTime a dateTime string specifying the stop of replay, or 'null'     
	...
	throws IOException, INMException {
	trace("createSubscription: stream="+streamName+
              " filter="+eventFilter.toXMLString()+
     ...
     * @param startTime a dateTime string specifying the replay start, or 'null'
     * @param stopTime a dateTime string specifying the stop of replay, 
     * or 'null'     
     * @see #receiveNotification()
     */
    public void createSubscription(String streamName,String eventFilter,
				   String startTime,String stopTime) 
	throws IOException, INMException {
	trace("createSubscription: stream="+streamName+" filter="+eventFilter+
	      " from="+startTime+" to="+stopTime);
        if (!capabilities.hasNotification())
	    throw new INMException(
                INMException.SESSION_ERROR,
     ...
     * <pre>
     *    Element subtree = 
     *        Element.create("urn:ietf:params:xml:ns:netmod:notification",
     *                       "netconf/streams");
     *    return session.get( subtree );
     * </pre>
     * The available streams are returned.
     */
    public NodeSet getStreams() throws INMException,IOException {
	Element filter = 
	    Element.create("urn:ietf:params:xml:ns:netmod:notification",
                           "netconf/streams");
	return get( filter );
     ...
     * the caller until an entire notifications messages has been
     * received. It's possible to check if there is data to be read
     * ahead using the ready() method on the SSHSession object.
     *
     */ 

    public Element receiveNotification() throws IOException, INMException  {
	StringBuffer notification = in.readOne();
	trace("notification= "+ notification);
	if (notification.length()==0)
	    throw new INMException(INMException.PARSER_ERROR,"empty input");	
     ...
     * parse an <ok/>. 
     * If not ok then throw RCP_REPLY_ERROR exception.
     */
    void recv_rpc_reply_ok() throws INMException, IOException {
     ...
     * Reads one rpc-reply from session and 
     * parse the <data>.
     * Returns the NodeSet contained in the data tag.
     */
    NodeSet recv_rpc_reply_data(int mid) throws INMException, IOException {
        return recv_rpc_reply("/data", parser, Integer.toString(mid));
    }
    
    NodeSet recv_rpc_reply_lockPartial(int mid) throws INMException, IOException {
        return recv_rpc_reply("", parser, Integer.toString(mid));
    }

    NodeSet recv_call_rpc_reply(Element e, int mid)
        throws INMException, IOException {
	com.tailf.inm.XMLParser parser = new com.tailf.inm.XMLParser(); //Why new parser?
        return recv_rpc_reply("", parser, Integer.toString(mid));
    }
    
    NodeSet recv_rpc_reply(String path) throws INMException, IOException {
	...
	if (data!=null) {	
	    ...
	    if (ctxtPrefix == null) ctxtPrefix = t.prefixes;
	    else ctxtPrefix.merge( t.prefixes );
	    if (ctxtPrefix == null) ctxtPrefix = new PrefixMap();
	    /* need to set parent of each data entry to null 
	     * don't want rpc-reply to be part of returned tree */
     ...
     * initial hello message so this method need to 
     * invoked before the {@link #hello()} method to 
     * have any effect.
     ...
     * The capabilities advertised from the client side
     * are the base NETCONF capability.
     *
     */
     ...
     * provided output transport.
     * This method is provided to be able extend this class
     ...
     * Encodes the RPC header and writes it to the 
     * provided output transport.
     * This method is provided to be able to extend this class
     * with proprietary capabilities.
     * The extra attribute arguement makes it possible to 
     * add an extra attribute to the RPC header.
     *
     * @param out Transport output stream
     ...
     *    String prefix = 
     *        Element.defaultPrefixes.nsToPrefix(Element.NETCONF_NAMESPACE);
     *    nc = mk_prefix_colon(prefix);
     */
    private String nc;
     ...
     * </rpc>
     */
    int encode_rpc(Transport out,Element data) throws INMException {
    ...
    int encode_getConfig(Transport out, String source,String xpath) {
        ...
        if (xpath != null && xpath.length() > 0) {
            out.print("<"+nc+"filter "+nc+"type=\"xpath\" "+nc+"select=\"");
            out.print(xpath);
            out.println("\"/>");
        }
        out.println("</"+nc+"get-config>");
     ...
     * Encode datastore
     */
    String encode_datastore(int datastore) throws INMException {
	String prefix = 
            Element.defaultPrefixes.nsToPrefix(Element.NETCONF_NAMESPACE);
	nc = mk_prefix_colon(prefix);

	switch(datastore) {
	case RUNNING:   return "<"+nc+"running/>";
	case CANDIDATE: return "<"+nc+"candidate/>";
	case STARTUP:   return "<"+nc+"startup/>";
	}
	throw new INMException(INMException.SESSION_ERROR,
     ...
     * Encode URL
     */
    String encode_url(String url) throws INMException {
	if (!isUrlOK(url))
	    throw new INMException(
                INMException.SESSION_ERROR,
	...
	if (urlSchemes==null) return false;
	for (int i=0;i<urlSchemes.length;i++) {
	    /* check if given url starts with any of the scheme urls
	     * given for the :url capability. for example "file:"
	     */	    
	    if (url.startsWith( urlSchemes[i]+":" )) return true;
	}
	return false;
    }
     ...
     * </rpc>
     */
    int encode_get(Transport out,Element subtreeFilter) throws INMException {
	int mid = encode_rpc_begin(out,withDefaultsAttr);
	out.println("<"+nc+"get>");
	out.println("<"+nc+"filter "+nc+"type=\"subtree\">");
	subtreeFilter.encode( out, true, capabilities );
     ...
     */
    void encode_defaultOperation(Transport out) 
	throws INMException {
	...
	default:
	    throw new INMException(INMException.SESSION_ERROR,
				    "unknown default-operation value: "
				    +defaultOperation);
	}
    }
     ...
     */
    void encode_testOption(Transport out) throws INMException {
	switch (testOption) {
	case NOT_SET: 
	    return;
	case SET: 
	...
	case TEST_THEN_SET:
		...
		throw new INMException(
                    ...
                    "test-option is given but the :validate "
                    +"capability is not supported by server");
	    out.println("<"+nc+"test-option>test-then-set</"+nc+"test-option>");
	    return;
	case TEST_ONLY:
            if (!capabilities.hasValidate())
		throw new INMException(
                    ...
                    "test-option is given but the :validate "
                    +"capability is not supported by server");
	    out.println("<"+nc+"test-option>test-only</"+nc+"test-option>");
	    return;
	default:
	    throw new INMException(
                INMException.SESSION_ERROR,
     ...
     */
    void encode_errorOption(Transport out) throws INMException {
	switch (errorOption) {
	case NOT_SET:
	    return;
	case STOP_ON_ERROR: 
     ...
     *
     */    
    int encode_validate(Transport out,Element configTree) throws INMException {
	int mid = encode_rpc_begin(out);
    ...
    /** 
     * Help function to make prefix.
     * Returns either:  "PREFIX:" or "".
     */
    ...
    void check_mid(Element t, String mid) throws INMException {
        if (mid == null) {
            return;
        }
        ...
        if (returned_id == null || (!returned_id.equals(mid))) {
            throw new INMException(
                    INMException.MESSAGE_ID_MISMATCH,
                    "After sending rpc with message-id=" + mid +
                    ", received rpc-reply with message-id=" + returned_id);
        }
    }
}
