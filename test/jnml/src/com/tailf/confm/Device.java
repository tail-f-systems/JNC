 ...
 * open for NETCONF notifications
 ...
 * </pre>
 * <p>
 * Once we have a new session, we also get a configTree (Element)
 * that we can use to accumulate changes in for the session. This
 * usually handy since the manager code typically consists of bits
 * and pieces that manipulate different parts of the configuration.
 ...
 * with the configuration change can be saved and
 * run later when the device comes up again.
 * </p>
 */
...
public class Device 
     ...
     * The backlog is saved when a configuration change is made
     * and the device is down, so that it can be re-sent later
     * when the device comes up again.
     *
     */
     ...
     * Note, this does not close the socket, it closes the SSH channel
     * which is a considerably cheaper operation.
     * The method also clears any accumulated configuration data we have.
     * @param sessionName symbolic Name of the session
     *
     */
    ...
    /**
     * end all NETCONF sessions and
     * close the SSH socket associated to this device
     * It also clears all accumulated config trees.
     */
     ...
     *                  getSession();
     * </pre>
     * This is required if we wish to monitor the ganymed Session object
     * for EOF
     */
     ...
     * wish to check if an ssh session is ready to read.
     */
     ...
     * This is not the same thing as the readTimeout. The connectTimeout
    ...
    public void connect(String localUser, int connectTimeout) 
        ...
        for (int i=0; i<users.size(); i++) {
            ...
            if (u2.localUser.equals(localUser)) {
                u = u2;
                break;
            }
        }
     ...
     * the backlog functionality. A typical sequence towards
     * a device is connect(), newSession(). If connect fails,
     * because the device is down, we still might want to have a
     * config tree to accumulate changes in although we have no connection
     * to the device. The getConfig(String) method searches in a list
     * of configTrees, thus we can have named configTrees although there is no
     * existing session towards the device.
     */
     ...
     * corresponds to one SSH channel towards the agent
     * @param sessionName symbolic Name of the session
     *
     */
	...
	newSessionConfigTree(sessionName);
        ...
        XMLParser parser = new com.tailf.confm.XMLParser();
        SSHSession sshSession = new SSHSession(con, defaultReadTimeout);
        if (sub != null)
            sshSession.addSubscriber(sub);
        ConfDSession session = new ConfDSession(sshSession, parser);
    ...
    private SessionConnData getConnData(String sessionName) {
        for (int i=0; i<connSessions.size(); i++) {
            SessionConnData p = (SessionConnData)connSessions.get(i);
            if (p.sessionName.compareTo(sessionName) == 0)
                return p;
        }
        return null;
    }
